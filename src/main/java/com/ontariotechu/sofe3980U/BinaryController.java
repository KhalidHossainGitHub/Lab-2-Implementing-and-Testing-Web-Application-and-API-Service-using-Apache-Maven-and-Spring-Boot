	...
	@RequestParam(name="operand2", required=false, defaultValue="") String operand2, Model model) {
        ...
        switch (operator) {
			...
			case "|":
				...
				break;
			default:
				// If no valid operator is found, set an error message in the model
				model.addAttribute("error", "Invalid operator");
				return "error"; // Return the calculator view in case of an error
		}
		
		return "result"; // Return the result page after the operation
